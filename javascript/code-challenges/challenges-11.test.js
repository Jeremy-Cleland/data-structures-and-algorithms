  ...
  input.map((arr) => {
    ...
    arr.map((num) => {
      if (num % 5 === 0 && typeof num === "number") {
        newArrTwo.push(Math.pow(2, num));
      }
    });
    result.push(newArrTwo);
  });
...
let findMaleAndFemale = (data) => {
  ...
  return result.map((character) => character.name).join(" and ");
};
...
let findShortest = (data) => {
  let shortestChar = data.reduce((acc, obj) => {
    if (acc.height < obj.height) {
      acc = obj;
    }
    return acc;
  });
